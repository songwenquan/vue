<template>
  <div class="fzcbjlph flex-col">
    <div class="title_content flex-bet">
      <p class="lineTitle">非正常办结率排行</p>
      <div class="tab">
...
export default {
  ...
  data() {
    return {
          ...
          render: (createElement, item) => {
            return createElement('div', [
              createElement('span', {
                ...
                domProps: {
                  innerHTML: item.row.abnormalTransactRate.toFixed(2),
                },
              }),
            ]);
          },
      ...
      defaultSort: { prop: 'abnormalTransactRate', order: 'descending' },
    };
  },
  methods: {
    // 处理列表数据
    handleData(data) {
      data.forEach((item) => {
        item.abnormalTransactRate = Number(item.abnormalTransactRate);
      });
      return data;
    },
    tabClick(val) {
