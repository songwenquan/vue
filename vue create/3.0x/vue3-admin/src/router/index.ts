...
function fullScreen(to: RouteLocationNormalized, next: NavigationGuardNext, url: any = '') {
  let title:any = to.meta.title ? to.meta.title : "管理平台"
  document.title = to.meta.title ? i18n.global.t(title) : "管理平台"
...
async function menusFunc(to: RouteLocationNormalized, next: NavigationGuardNext) {
	...
	if (process.env.NODE_ENV === 'production' && to.path !== '/permissionMenu' && checkPermission('menuUrl', to.path, menus) === -1) {
		// 没有该菜单权限
...
router.beforeEach(async (to: RouteLocationNormalized, from: RouteLocationNormalized, next: NavigationGuardNext) => {
	...
	// 判断是否为需要登录的页面
	if (to.meta.requireAuth === false || to.path === '/' || to.path === '/login') {
		fullScreen(to, next);
	} else {
